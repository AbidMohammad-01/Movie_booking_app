...
const resetpassword = async(req,res)=>{
 ...
 const user=  await User.findOne(
            {resettoken:token,resettime:{$gt:Date.now()}}
        )
    
        let hashed = await bcryptjs.hash(password,5) 
        const update={ resettoken : undefined,
            resettime: undefined,password:hashed} 
...
const forgotpassword = async(req,res)=>{
  ...
  const update={ resettoken : token,
    ...
    from:{
        ...
        We recieved a request to rest your password for your account.
        ...
        This link will be expire in 24 hours.Please contact our support team regarding any queries
